      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {/* Type Tab Selector */}
            <div className="grid grid-cols-2 gap-2 mt-4 p-1.5 rounded-xl bg-slate-950/70 border border-slate-800">
              <button
                onClick={() => {
            ...
            {(isAdding || editingId) && (
              <motion.form
                ...
                onSubmit={handleSave}
                className="mt-4 p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3"
              >
                ...
                {errorMsg && (
                  <p className="text-xs text-rose-400 font-medium">⚠️ {errorMsg}</p>
                )}

                <div>
                  <label className="text-xs text-slate-400 block mb-1">Nama Kategori</label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Misal: Bonus, Tabungan, Listrik, Hiburan..."
                    className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white text-sm focus:outline-none focus:border-emerald-500"
                    required
                    autoFocus
                  />
                </div>
                ...
                {/* Icon Selection */}
                <div>
                  <label className="text-xs text-slate-400 block mb-1.5">Pilih Ikon</label>
                  <div className="grid grid-cols-8 gap-1.5 max-h-24 overflow-y-auto p-1 bg-slate-900 rounded-lg border border-slate-800">
                ...
                {/* Color Selection */}
                <div>
                  <label className="text-xs text-slate-400 block mb-1.5">Pilih Warna</label>
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {cat.is_default && (
                          <span className="text-[10px] text-slate-500 font-medium">Bawaan Sistem</span>
                        )}
                      </div>
                    </div>
